...
Предварительно ознакомиться: [материалы лекции по командам CREATE TABLE и INSERT](https://github.com/db-course/lecture-notes/blob/master/04/lecture4.md).
...
При создании таблиц следует пользоваться системой типов данных, определенной в стандарте языка SQL, и не ориентироваться на типизацию SQLite, которая сильно от нее отличается. Поскольку SQLite толерантно относится к указываемым для колонок типам, это не должно привести к проблемам. Иными словами, создание таблиц должно происходить так, как будто используется не SQLite, а любая СУБД, соответствующая стандарту.
