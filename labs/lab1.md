...
Освоение методов концептуального моделирования при проектировании схемы данных для реляционной базы данных.

### Теоретические сведения

**Концептуальное моделирование** — способ разработки структуры базы данных, который опирается на семантику (смысл) данных. Входными данными является словесное описание предметной области, полученное от экспертов, выходными — формализованная модель, как правило, представленная в виде ER-диаграммы.

#### Общие понятия и принципы концептуального моделирования [1]

**ER-модель** (Entity-Relationship model, модель Сущность-Связь) описывает предметную область как набор сущностей, семантически связанных между собой. Графически изображается в виде ER-диаграммы.
...
В случаях, когда между сущностями существует единственная связь кратности "один-к-одному", часто эти сущности могут быть отождествлены с точки зрения данных и слиты в одну, если этому нет семантических или иных препятствий. Например, сущности "Товар" и "Товар на складе" могут быть отождествлены в том случае, если склад единственен, и одному товару соответствует ровно один факт его наличия на этом складе.
...
В данной лабораторной работе в академических целях предпочтение необходимо отдавать естественным ключам. Допускается использование суррогатного ключа при обосновании его необходимости.
...
Иногда у сущности может быть выделено несколько естественных ключей. В учебных работах достаточно выделения одного.

#### Переход к реляционной модели

Предварительно ознакомиться: [материалы лекции по реляционной модели данных](https://github.com/db-course/lecture-notes/blob/master/01/lecture1.md).
...
### Типичные ошибки при концептуальном моделировании

Ниже перечислены ошибки, которые регулярно встречаются при выполнении лабораторной работы.

1. Смешение сущности и атрибута

**Ошибка**:	

Сущность, обладающая собственной семантикой и потенциальными связями, моделируется как атрибут.

**Пример**:

Сущность "Студент" с атрибутом "Группа". 

**Правильный принцип**:

Если группа имеет собственные свойства (номер, "Год поступления") либо связи (например с сущностью "Пара") — это отдельная сущность.

2. Хранение списков в атрибутах

**Ошибка**:

Атрибут содержит набор значений (нарушается атомарность).

**Пример**:

Атрибуты вида "телефоны", "актерский состав" и т.д.

**Правильный принцип**:

Либо изменить гранулярность сущности, либо вынести значения в отдельную сущность и создать связь.

3. Отсутствие ключа или неверный выбор ключа

**Ошибка**:

Сущность не имеет явно определённого ключа или ключ не гарантирует уникальность.

**Пример**:

Использование в качестве ключа неуникальных атрибутов (например, "ФИО"). Использование составного ключа без анализа минимальности.

**Правильный принцип**:

Ключ должен быть:
* минимальным,
* гарантированно уникальным,
* семантически обоснованным.

6. Моделирование процессов вместо структуры данных

**Ошибка**:

В модель включаются действия и события как процессы, хотя требуется описывать данные.

**Пример**:

Сущности "Оформление заказа", "Регистрация пользователя".

**Правильный принцип**:

ER-модель описывает состояние данных ("Заказ", "Пользователь"), а не алгоритм их изменения.
...
1. [Атомарность данных](https://wiki.loginom.ru/articles/atomic-data.html)

### Вопросы к защите

*   В чем отличия концептуальной и логической моделей?
