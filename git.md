...
```
`ваше имя` - имя, которым будут подписаны ваши изменения. Рекомендуется выбрать такое имя, чтобы оно совпадало с вашим именем, указанным в профиле на GitHub. Например,

```bash
git config --global user.name "Aleksei Aksenov"
```
...
Для того чтобы выполнять лабораторные работы, нужно быть членом организации `db-course` на GitHub. Для этого необходимо после регистрации на GitHub подать заявку на вступление, которую принимает преподаватель. Для этого необходимо открыть issue (проблему) в репозитории [db-course/students](https://github.com/db-course/students). Как это сделать, указано в описании репозитория.

Преподаватель высылает вам на указанный при регистрации на GitHub адрес электронной почты приглашение на вступление в организацию. Его необходимо принять.
...
Для тренировки работы с git и GitHub предназначена 0ЛР (тренировочная ЛР). Она является обязательной, но ее не требуется защищать и по ней не нужно оформлять отчет. Первоначально члену организации доступен репозиторий только этой лабораторной работы (`db-course/labwork0`), а после ее выполнения становятся доступны остальные репозитории.

Чтобы выполнить тренировочную лабораторную работу, необходимо проделать те же действия, которые предусмотрены для обычных лабораторных работ: форк, клонирование, создание коммитов, отправка изменений, создание пулл-реквеста (см. следующие разделы инструкции).

Следует помнить, что репозиторий 0ЛР, в отличие от репозиториев всех остальных работ, не имеет разделения на группы, поэтому для доступа к нему следует использовать имя `ваше_имя/labwork0`, а не `ваше_имя/labwork0-4141` или подобное.

Для выполнения 0ЛР требуется создать 2 коммита:

1. Создание одиночного текстового файла bio.txt, содержащего на первой строке вашу фамилию, имя, номер группы
2. Добавление в файл bio.txt на вторую строку вашей краткой биографии (не более 30 слов). Если вы не имеете желания указывать факты своей биографии, можно заменить ее на описание вашего любимого сериала или компьютерной игры.

## Подготовка к выполнению лабораторной работы

Необходимо форкнуть (fork) приватный репозиторий, относящийся к лабораторной работе, которую вы хотите сдать. Для этого необходимо зайти в репозиторий, соответствующий работе (например, `db-course/labwork3-4141`, если вы учитесь в группе 4141 и сдаете ЛР3), и нажать кнопку Fork.
...
Например, если студент группы 4141 с именем пользователя `ivanov` собирается клонировать репозиторий с 3-й ЛР, для того чтобы сдать ее, необходимо выполнить команду

```bash
git clone https://github.com/ivanov/labwork3-4141
```
...
Будет создан локальный каталог (в данном случае `labwork3`), содержащий файл `README.md` (текстовый файл с описанием репозитория в формате markdown) и, другие файлы, если это предусмотрено лабораторной работой. Можно приступать к выполнению лабораторной работы.
...
Для создания пулл-реквеста необходимо зайти на страницу своего репозитория-форка, открыть вкладку "Pull requests" и нажать зеленую кнопку New pull request.
...
![Alert](https://github.com/db-course/syllabus/blob/master/img/alert.png)

В случае успешной сдачи преподаватель отмечает это соответствующим комментарием и закрывает пулл-реквест.
