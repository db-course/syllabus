...
```
`ваш@email` - адрес электронной почты. Чтобы коммиты, которые вы создаете, ассоциировались с вашим пользователем на GitHub, желательно использовать адрес электронной почты, привзянный к GitHub, либо, если нет желания "светить" реальный адрес, использовать адрес генерируемый GitHub и имеющий вид `ivanov@users.noreply.github.com`, где `ivanov` -- ваше имя пользователя.
...
Поскольку это приватный репозиторий, клиент запросит авторизацию, чтобы проверить, обладаете ли вы правами на чтение из него. 

Для аутентификации требуется [сгенерировать токен доступа](https://github.com/settings/tokens/new), чтобы использовать его вместо вашего пароля от GitHub. При создании токена задайте для него группу разрешений `repo`.

При использовании Git Bash, GNU/Linux или macOS логин и пароль будут сохранены с помощью менеджера учетных данных, и при последующем исполнении команд вводить их повторно не потребуется.

В WSL необходимо выполнить

```bash
git config --global credential.helper store
```

для того, чтобы включить запоминание учетных данных.

В случае успешной авторизации репозиторий будет склонирован.

В текущем каталоге будет создан новый подкаталог (в данном случае `lab3-4241`), содержащий файл `README.md` (текстовый файл с описанием репозитория в формате markdown) и, другие файлы, если это предусмотрено лабораторной работой. Можно приступать к выполнению лабораторной работы.
...
Эта команда еще страшнее, чем предыдущая, поскольку **она удаляет не только все незафиксированные изменения, но и данные последней фиксации**!
...
_Решение_. Использовать `git reset`, также как и в предыдущем случае, нельзя, поскольку изменения были опубликованы. Хорошая новость в том, что для того чтобы поменять содержимое пулл-реквеста, нужно просто опубликовать исправленный коммит и, если необходимо, дополнения. Решение полностью совпадает с предыдущим.
